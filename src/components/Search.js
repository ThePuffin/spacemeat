...
class Search extends Component {
  ...
  render() {
    ...
    const page1 = this.state.api.map((elt, i) => (
      <CardPute
        height={elt.height}
        name={elt.name}
        eyeColor={elt.eyeColor}
    ...
    console.log(page1);
    const page2 = this.state.filtreApi.map((elt, i) => {
      <CardPute
        height={elt.height}
        name={elt.name}
        eyeColor={elt.eyeColor}
    ...
    return page2.length > 0 ? (
      <div>
        ...
        {page2[this.state.compteur]}
      </div>
    ) : (
      <div>
        <h1>Choose your meat</h1>
        <CheckBox maj={this.update} pression={this.tri} />{" "}
        {page1[this.state.compteur]}
      </div>
    );
  }
>>>>>>> 6f7a97deaca90c7536ed543ebae7b44a6b776c3d
}
